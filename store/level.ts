...
export const useLevelStore = defineStore('level', () => {
  ...
  const currentExpPercent = computed(() =>
    isPercent.value
      ? exp.value
      : Math.round((currentExp.value / maxExp.value) * 100).toFixed(3),
  );
  ...
  const afterExpPercent = computed(() =>
    afterLevel.value < 300
      ? +Math.round((afterExp.value / afterMaxExp.value) * 100).toFixed(3)
      : 100,
  );
  ...
  const afterLevelMinusExp = (exp: number) => {
    let tempLevel = afterLevel.value;
    let tempExp = exp;
    let totalExp = getTotalExp(tempLevel) + afterExp.value;
    let minus = totalExp - tempExp;
    if (tempLevel > 299) {
      afterLevel.value = 299;
      afterExp.value = totalExp - minus;
      return;
    }
    const info = LevelExp.find((item) => item.totalExp >= minus);
    if (info) {
